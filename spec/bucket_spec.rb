...
describe Ihasa::Bucket do
  ...
  context 'with a standard bucket' do
    ...
    describe '#guard' do
      it { expect(bucket.guard).to eq true }
      it 'sustain burst' do
        10.times { expect(bucket.guard).to eq true }
      end
      it 'no longer reponsd true once the burst limit is crossed' do
        10.times { expect(bucket.guard).to eq true }
        expect(bucket.guard).to eq false
      end
      it 'regenerate its allowance over time' do
        skip "Slow integration test"
        10.times { expect(bucket.guard).to eq true }
        10.times { expect(bucket.guard).to eq false }
        sleep 2.1
        10.times { expect(bucket.guard).to eq true }
        10.times { expect(bucket.guard).to eq false }
        sleep 1.1
        5.times { expect(bucket.guard).to eq true }
        sleep 1
        5.times { expect(bucket.guard).to eq true }
        sleep 1
        5.times { expect(bucket.guard).to eq true }
      ...
      it 'execute the given block only when the burst limit is not met' do
        count = 0
        10.times { |i| expect(bucket.guard { count += 1 }).to eq(1 + i) }
        expect(count).to eq 10
        10.times { bucket.guard { count += 1 } }
        expect(count).to eq 10
      end
    end
    describe '#guard!' do
      ...
      it 'no longer reponsd true once the burst limit is crossed' do
        10.times { expect(bucket.guard!).to eq true }
        expect{bucket.guard!}.to raise_error(/Bucket IHAB throttle limit/)
      end
    end
